...
public class BookPanel extends JPanel {
    ...
    public BookPanel(DataStorage dataStorage) {
        ...
        // Desktop pane untuk menampilkan PDF
                        ...
                        if (book != null && book.isDigital()) {
                            try {
                                ...
                                if (pdfFile.exists()) {
                                    ...
                                    JLabel pageLabel = new JLabel("Page 1 of " + document.getNumberOfPages());
                                    int[] currentPage = {0}; // Array untuk menyimpan halaman saat ini
                                    navPanel.add(prevButton);
                                    navPanel.add(pageLabel);
                                    navPanel.add(nextButton);
                                    pdfPanel.add(navPanel, BorderLayout.SOUTH);

                                    // Render halaman pertama
                                    BufferedImage image = pdfRenderer.renderImageWithDPI(currentPage[0], 100);
                                    ...
                                    prevButton.addActionListener(e -> {
                                        if (currentPage[0] > 0) {
                                            currentPage[0]--;
                                            try {
                                                BufferedImage prevImage = pdfRenderer.renderImageWithDPI(currentPage[0], 100);
                                                pdfLabel.setIcon(new ImageIcon(prevImage.getScaledInstance(600, 800, Image.SCALE_SMOOTH)));
                                                pageLabel.setText("Page " + (currentPage[0] + 1) + " of " + document.getNumberOfPages());
                                            } catch (IOException ex) {
                                                JOptionPane.showMessageDialog(null, "Gagal membuka halaman: " + ex.getMessage());
                                            }
                                        }
                                    });

                                    nextButton.addActionListener(e -> {
                                        if (currentPage[0] < document.getNumberOfPages() - 1) {
                                            currentPage[0]++;
                                            try {
                                                BufferedImage nextImage = pdfRenderer.renderImageWithDPI(currentPage[0], 100);
                                                pdfLabel.setIcon(new ImageIcon(nextImage.getScaledInstance(600, 800, Image.SCALE_SMOOTH)));
                                                pageLabel.setText("Page " + (currentPage[0] + 1) + " of " + document.getNumberOfPages());
                                            } catch (IOException ex) {
                                                JOptionPane.showMessageDialog(null, "Gagal membuka halaman: " + ex.getMessage());
                                            }
