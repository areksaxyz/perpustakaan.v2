        ...
        if (isPushed) {
            ...
            if (row >= 0) {
                ...
                if (now.after(transaction.getDueDate()) && transaction.getReturnDate() == null) {
                    clickCount++; // Hitung jumlah klik
                    double fine = clickCount * 5000.0; // Rp 5.000 per klik
                    transaction.setFine(fine);
                    dataStorage.getFines().add(transaction);
                    JOptionPane.showMessageDialog(null, "Denda ditambahkan: Rp " + fine);
                    // Perbarui tabel FinesPanel (asumsi FinesPanel diperbarui secara manual)
                } else {
